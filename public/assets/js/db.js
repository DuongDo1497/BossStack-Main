const listTitlePage = [
  {
    titlePage: "Về BossStack",
    description: "",
    link: "https://bossstack.vn/about-us",
  },
  {
    titlePage: "Phần mềm Kiểm soát thất thoát",
    description: "Quản lý mức độ hoàn thành tác vụ của nhân viên",
    link: "https://bossstack.vn/loss-control",
    button: "Liên hệ",
    imageLink: "https://bossstack.vn/image/banner-feature.png",
  },
  {
    titlePage: "Coaching",
    description:
      "Giúp doanh nghiệp và chủ doanh nghiệp xây dựng chiến lược Quản lý tài chính và Kiểm toán vốn",
    link: "https://bossstack.vn/coaching",
    button: "Tìm hiểu thêm",
    imageLink: "https://bossstack.vn/image/banner-feature-1.png",
  },
  {
    titlePage: "Phần mềm Hỗ trợ quyết định",
    description:
      "Tự đông hóa quy trình đánh giá và xây dựng kế hoach tài chính cho doanh nghiệp.",
    link: "https://bossstack.vn/decision-support",
    button: "Liên hệ",
    imageLink: "https://bossstack.vn/image/banner-feature-2.png",
  },
  {
    titlePage: "Tuyển dụng",
    description: "",
    link: "https://bossstack.vn/recruitment",
  },
  {
    titlePage: "Tuyển dụng",
    description: "",
    link: "https://bossstack.vn/recruitment-detail",
  },
  {
    titlePage: "Blog BossStack",
    description:
      "Lý do các công ty chọn giải pháp công nghệ của BossStack trong quản lý tài chính và vận hành doanh nghiệp",
    link: "https://bossstack.vn/blog",
  },
  {
    titlePage: "Khách hàng BossStack",
    description:
      "Lý do các công ty chọn giải pháp công nghệ của BossStack trong quản lý tài chính và vận hành doanh nghiệp",
    link: "https://bossstack.vn/client",
  },
  {
    titlePage: "Công ty nút áo Tôn Văn",
    description:
      "Công ty TNHH Nút áo Tôn Văn là doanh nghiệp chuyên sản xuất, gia công nút áo, đồ mỹ nghệ gia dụng bằng vỏ ốc, sò, gỗ, đá, kim loại. Công ty có nhà xưởng rộng tới 10.000 m2 và hơn 120 công nhân lành nghề.",
    link: "https://bossstack.vn/client-detail",
  },
  {
    titlePage: "Liên hệ",
    description: "",
    link: "https://bossstack.vn/contact",
  },
];

const recruitmentListdb = [
  {
    position: "Tuyển dụng leader Kế toán",
    dealine: "29/09/2022",
  },
  {
    position: "Tuyển dụng nhân viên Back - End",
    dealine: "29/09/2022",
  },
  {
    position: "Tuyển dụng nhân viên Marketing",
    dealine: "29/09/2022",
  },
  {
    position: "Tuyển dụng chuyên viên Font - end",
    dealine: "29/09/2022",
  },
  {
    position: "Tuyển dụng UI/UX Designer",
    dealine: "29/09/2022",
  },
  {
    position: "Tuyển dụng nhân viên Graphic Designer",
    dealine: "29/09/2022",
  },
];
